...
import ass5contest
import pacman
from pacman.tech_util import injection_decorator

# the directory of the layout files
LAYOUT_FILES_DIR = pathlib.Path('pacman/layout_files')

# the names of all the layout files
layout_names = sorted([f.stem for f in LAYOUT_FILES_DIR.glob('*')])
contest_layouts = [f for f in layout_names if f.startswith('contest')]
...
# all available search heuristics
...
search_agent = injection_decorator(search_agents)
search_method = injection_decorator(search_methods)
search_representation = injection_decorator(search_representations)
search_heuristic = injection_decorator(search_heuristics)
reflex_agent = injection_decorator(reflex_agents)
adversarial_agent = injection_decorator(adversarial_agents)
adversarial_evaluate = injection_decorator(adversarial_evaluates)
...
def run(layout_name: str, pacman_agent: pacman.agents.PacmanAgent, display: pacman.displays.Display,
    ...
    # create ghost agents of correct type
    ghost_agents = [ghosts[i][0].agent(i + 1) for i in range(num_ghosts)]
    agents = [pacman_agent] + ghost_agents

    return pacman.game.run_game(layout, agents, display, speed, timeouts)


def main(ask_commands=True):
    # global arguments
    ...
    if ask_commands and len(sys.argv) == 1:
        sys.argv.extend([x for x in re.split(
            r' +', input("Enter command-line arguments: ")) if x != ''])
    ...
    if not args.mode or args.mode == 'keyboard':
        ...
        if args.heuristic:
            ...
            def method(state): return search_methods[args.method](
                state, heuristic)
    ...
    for irun in range(args.runs):
        for ilayout, layout in enumerate(layouts):
            ...
            # Run game
            gstate, stats = run(layout, pacman_agent, display,
                                args.speed, args.ghosts, args.timeouts)
...
def command(command):
    sys.argv = ['run.py'] + command.split()
    main(ask_commands=False)
